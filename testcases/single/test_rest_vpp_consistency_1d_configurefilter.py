...
class TestRestVppConsistency1DConfigureFilter(unittest.TestCase):
    ...
    def test_get_configobjs_and_stateobjs_filter(self):
        ...
        # 过滤得到link 1
        resp = self.topo.dut1.get_rest_device().do_get_configs_request("Link", filter="Filters[LinkId][eq]=1")
        glx_assert(resp.status_code == 200)
        data = json.loads(resp.text)
        glx_assert(data["ObjCount"] == 1)
        glx_assert(data["Objects"][0]["Object"]["LinkId"] == 1)
        resp = self.topo.dut1.get_rest_device().do_get_states_request("Link", filter="Filters[LinkId][eq]=1")
        glx_assert(resp.status_code == 200)
        glx_assert(data["ObjCount"] == 1)
        glx_assert(data["Objects"][0]["Object"]["LinkId"] == 1)
        ...
        resp = self.topo.dut1.get_rest_device().do_get_configs_request("Link", filter="Filters[TunnelId][eq]=12&[Tag1][eq]=kk")
        glx_assert(resp.status_code == 200)
        data = json.loads(resp.text)
        glx_assert(data["ObjCount"] == 2)
        glx_assert(data["Objects"][0]["Object"]["LinkId"] == 1)
        glx_assert(data["Objects"][1]["Object"]["LinkId"] == 2)
        resp = self.topo.dut1.get_rest_device().do_get_states_request("Link", filter="Filters[State][eq]=registering&[TxBytes][eq]=0")
        glx_assert(resp.status_code == 200)
        glx_assert(data["ObjCount"] == 2)
        glx_assert(data["Objects"][0]["Object"]["LinkId"] == 1)
        glx_assert(data["Objects"][1]["Object"]["LinkId"] == 2)
        ...
        # 过滤tag2为1, 得到link 1
        resp = self.topo.dut1.get_rest_device().do_get_configs_request("Link", filter="Filters[Tag2][eq]=1")
        glx_assert(resp.status_code == 200)
        data = json.loads(resp.text)
        glx_assert(data["ObjCount"] == 1)
        glx_assert(data["Objects"][0]["Object"]["LinkId"] == 1)
    ...
    def test_update_tunnels_config_filter(self):
        ...
        tunnelTable = {}
        tunnelTable["Table"] = "Tunnel"
        tunnelTable["Filters"] = "Filter[Tag1][eq]=k&[TunnelId][eq]=2"
        tunnel1 = {}
        tunnel1["TunnelId"] = 1
        tunnel1["SchedMode"] = ""
        tunnel1["BandwidthLimit"] = 0
    ...
    def test_update_edge_routes_config_filter(self):
        ...
        # 删除
        self.topo.dut1.get_rest_device().delete_edge_route(route_prefix="192.168.1.1/32")
        self.topo.dut1.get_rest_device().delete_edge_route(route_prefix="192.168.1.3/32")
    ...
    def test_update_bizpols_config_filter(self):
        ...
        self.topo.dut1.get_rest_device().delete_bizpol(name="bizpol3")
