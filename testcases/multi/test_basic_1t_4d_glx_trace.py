...
class TestBasic1T4DGlxTrace(unittest.TestCase):
    ...
    def test_01_basic(self):
        # 为每个节点设置id
        resp = self.topo.dut1.get_rest_device().set_global_cfg(node_id=1)
        glx_assert(resp.status_code == 200)

        resp = self.topo.dut2.get_rest_device().set_global_cfg(node_id=2)
        glx_assert(resp.status_code == 200)

        resp = self.topo.dut3.get_rest_device().set_global_cfg(node_id=3)
        glx_assert(resp.status_code == 200)

        resp = self.topo.dut4.get_rest_device().set_global_cfg(node_id=4)
        glx_assert(resp.status_code == 200)


        src_route_label = "0x1200010"
        dst_route_label = "0x3400010"
        # 默认等待5秒钟
        out, err = self.topo.dut1.get_vpp_ssh_device().get_cmd_result(f"vppctl glx trace src-route-label {src_route_label} dst-route-label {dst_route_label}")
        glx_assert(err == '')
        # dut2 trace result
        glx_assert("node-role P ttl 255 tunnel-id 12" in out)
        # dut3 trace result
        glx_assert("node-role POP ttl 254 tunnel-id 23" in out)
        # dut4 trace result
        glx_assert("node-role Edge ttl 253 tunnel-id 34" in out)
        
        glx_assert("3 received" in out)

    def test_02_repeat(self):
        # 为每个节点设置id
        resp = self.topo.dut1.get_rest_device().set_global_cfg(node_id=1)
        glx_assert(resp.status_code == 200)
        ...
        dst_route_label = "0x3400010"
        # 默认等待5秒钟
        out, err = self.topo.dut1.get_vpp_ssh_device().get_cmd_result(f"vppctl glx trace src-route-label {src_route_label} dst-route-label {dst_route_label} repeat 3")
        glx_assert(err == '')
        # dut2 trace result
        glx_assert("node-role P ttl 255 tunnel-id 12" in out)
        # dut3 trace result
        ...
        glx_assert("9 received" in out)

    def test_03_edge_down(self):
        # 为每个节点设置id
        resp = self.topo.dut1.get_rest_device().set_global_cfg(node_id=1)
        glx_assert(resp.status_code == 200)
        ...
        src_route_label = "0x1200010"
        dst_route_label = "0x3400010"
        self.topo.dut4.get_vpp_ssh_device().get_cmd_result("systemctl stop fwdmd")
        self.topo.dut4.get_vpp_ssh_device().get_cmd_result("systemctl stop vpp")
        time.sleep(5)
        # 默认等待5秒钟
        # 等待策略老化
        time.sleep(30)
        out, err = self.topo.dut1.get_vpp_ssh_device().get_cmd_result(f"vppctl glx trace src-route-label {src_route_label} dst-route-label {dst_route_label}")
        glx_assert(err == '')
        # 不应该再收到dut4的trace reply了
        glx_assert("node-role Edge" not in out)
        # 策略老化就无法确认是什么角色
        glx_assert("node-role Unknown" in out)
        self.topo.dut4.get_vpp_ssh_device().get_cmd_result("systemctl start vpp")
        self.topo.dut4.get_vpp_ssh_device().get_cmd_result("systemctl start fwdmd")
        time.sleep(20)
